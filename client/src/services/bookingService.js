...
const bookingService = {
  ...
  assignTechnician: async (bookingId, technicianId) => {
    try {
      const response = await api.patch(`/bookings/${bookingId}/assign`, { technicianId });
      return response.data;
    } catch (error) {
  ...
  getAvailableTimeSlots: async (date) => {
    try {
      const response = await api.get(`/bookings/availability/timeslots?date=${date}`);
      return response.data;
    } catch (error) {
  ...
  getAvailableTechnicians: async (date, timeSlot) => {
    try {
      const response = await api.get(`/bookings/availability/technicians?date=${date}&timeSlot=${encodeURIComponent(timeSlot)}`);
      return response.data;
    } catch (error) {
      throw error.response?.data || { error: 'Failed to fetch available technicians' };
    }
  }
};
